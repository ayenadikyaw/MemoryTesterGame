...
const setCardColors = (indexes) => {
  for (let i = 0; i < indexes.length; i++) {
    cards[indexes[i]].style.backgroundColor = colors[i];
    indexColor[indexes[i]] = colors[i];
  }
};
...
function play(obj) {
  if (isClickable) {
    ...
    if (!clickedCards.includes(cardID)) {
      pairColor.push(indexColor[cardID]);
    ...
    if (clickCount == 2) {
      if (checkMatch()) {
        ...
        console.log("Turning back", clickedCards);
        setTimeout(() => {
...
function initialize() {
  ...
  if (choosenMode) {
    ...
    setCardColors(cardIndexes);
    //setTimeout(resetColors,3000);// Set up the first timer
    setTimeout(() => {
      resetColors(); // Reset colors after 3000ms
      setTimeout(() => {
        isClickable = true;
